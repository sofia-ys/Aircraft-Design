...
def plot_shear_force_distribution(x_vals, shear_force_vals):
    ...
    plt.yticks(np.arange(0, max(shear_force_vals) + max(shear_force_vals)/20, round(max(shear_force_vals)/20,-4)))  # Denser y-axis ticks
    plt.show()
...
def plot_bending_moment_distribution(x_vals, bending_moment_vals):
    ...
    plt.grid(which='major', linestyle='-', linewidth=0.7)
    plt.xticks(np.arange(0, 18.5, 0.5))  # Denser x-axis ticks
    plt.yticks(np.arange(0, max(bending_moment_vals) + max(bending_moment_vals)/20, round(max(bending_moment_vals)/20,-5)))  # Denser y-axis ticks
    plt.show()
