...
def CentroidZcontribution(AS, sb, st, alpha, n2, n1, d1, d2, d3, d4):
    CB_z = sum(AS * i * sb * math.sin(alpha) for i in range(0, n2)) #for bottom

    CB_x = sum(AS * i * sb * math.cos(alpha) for i in range(0, n2)) #top

    CT_x = sum(AS * i * st  for i in range(0, n1)) #top
    u=d4*math.tan(alpha)
    if d4 > 0:
